...
Analyze peptide docking to spheres, focusing on
...
def build_gas_phase_system(topology):
    ...
    return ff.createSystem(topology, nonbondedMethod=NoCutoff, constraints=HBonds, rigidWater=True)
...
def compute_binding_deltaG(rec_pdb, lig_pdb, nsteps, dt, interval, verbose=False):
    ...
    stderr = np.sqrt(E_c.var(ddof=1)/len(E_c) + E_r.var(ddof=1)/len(E_r) + E_l.var(ddof=1)/len(E_l))
...
def compute_metrics(pts, elems, centers, radii):
    ...
    frac_per = inside.sum(0)/N; frac = float(frac_per.max())
    depths = np.clip((radii[None] - np.sqrt(d2))/radii[None],0,1)
    depth = float(depths.max(1).mean()); score = frac * depth
    w = np.array([VdW_R.get(e,1.5)**3 for e in elems])
    wfrac = float((inside * w[:,None]).sum(0).max() / w.sum())
    wdepth = float((depths * w[:,None]).max(1).sum() / w.sum()); wscore = wfrac * wdepth
    bi = int(frac_per.argmax()); C = np.array([e=='C' for e in elems])
    hydro_frac = float((inside[:,bi] & C).sum() / max(C.sum(),1))
    atom_d = depths[:,bi]; uniformity = float(max(0.0,1-atom_d.std()))
    uniform_score = score * uniformity
    return frac, depth, score, wfrac, wdepth, wscore, hydro_frac, uniformity, uniform_score
...
if __name__ == '__main__':
    ...
    vdir = os.path.join(args.outdir, 'vicinity_ligs'); os.makedirs(vdir, exist_ok=True)
    ...
    for pdb in pdbs:
        ...
        if m[0] >= args.frac_thresh and m[1] >= args.depth_thresh:
            dG, se = compute_binding_deltaG(args.rec, pdb, args.nsteps, args.dt, args.interval, args.verbose)
            if np.isfinite(dG):
    ...
    for idx, e in enumerate(kept):
        lbl, name, frac, depth, score, wf, wd, ws, hf, unif, uscore, dG, stderr, cent = e
        final.append((lbl, name, frac, depth, score, wf, wd, ws, hf, unif, uscore, zuni[idx], dG, stderr, cent))
    ...
    sc = ax.scatter([e[2] for e in final], [e[3] for e in final], c=zlabels, cmap='plasma', s=60, edgecolors='k')
    ax.scatter(best[2], best[3], c='red', s=150, marker='*', label='Best')
    for lbl, x, y in zip(labels, [e[2] for e in final], [e[3] for e in final]): ax.annotate(lbl, (x,y), textcoords='offset points', xytext=(5,5), fontsize=9)
    ax.set(xlabel='Frac', ylabel='Depth', title='Frac vs Depth')
    fig.colorbar(sc, ax=ax, label='ZUniform'); ax.legend(); fig.tight_layout(); fig.savefig(os.path.join(args.outdir, 'A_frac_depth.png'))

    # Plot B
    fig, ax = plt.subplots(figsize=(8,4)); ax.bar(labels, [e[10] for e in final], edgecolor='k'); ax.set(xlabel='Pose',ylabel='UniformScore',title='UniformScore per Pose'); fig.tight_layout(); fig.savefig(os.path.join(args.outdir, 'B_uniformscore_bar.png'))

    # Plot C
    fig, ax = plt.subplots(figsize=(6,6)); ax.scatter(scores, wscores, edgecolors='k'); ax.plot([scores.min(),scores.max()],[scores.min(),scores.max()],'--'); ax.set(xlabel='Score',ylabel='WScore',title=f'Score vs WScore (r={r_val:.2f})'); fig.tight_layout(); fig.savefig(os.path.join(args.outdir, 'C_score_vs_wscore.png'))

    # Plot D
    fig, ax = plt.subplots(figsize=(6,4)); ax.hist(zlabels, bins=10, edgecolor='k'); ax.set(xlabel='ZUniform',ylabel='Count',title='UniformScore Z-Score Distribution'); fig.tight_layout(); fig.savefig(os.path.join(args.outdir, 'D_zuniform_hist.png'))
    ...
    fig.colorbar(sc2, ax=ax, label='ZUniform'); fig.tight_layout(); fig.savefig(os.path.join(args.outdir, 'E_score_vs_dG.png'))

    # Vicinity
    with open(os.path.join(vdir,'vicinity_ligs.log'),'w') as vf:
        vf.write('Idx,Name,Distance\n')
        for lbl,name,_,_,_,_,_,_,_,_,_,_,_,_,cent in final:
            dist = np.linalg.norm(np.array(cent) - best_cent)
            vf.write(f"{lbl},{name},{dist:.3f}\n")
        vf.write(f"Total,{len(final)}\n")

    print(f"Done: {len(final)} kept; best={best[0]} ZU={best[11]:.2f}")
